...
the `mediation` R package and mirrored by
...
record post-treatment depressive symptoms and employment-related measures.
...
capable of conducting a successful search. The DAG helps separate the policy
question, "does assignment help?", from the mechanism question, "is part of the
effect transmitted through self-efficacy?"
...
illustrative analysis file. The original randomized study was larger; for
substantive claims about program efficacy one should return to the original
study and documentation. Here the goal is to show a complete causal workflow:
data, graph, identification, total-effect estimation, and a clearly labeled
...
## 5. Optional: Using Crumble.jl for Mediation Estimation
...
1. use `CausalGraphs.jl` to state and stress-test the DAG assumptions;
2. use `identify()` to separate total-effect identification from mechanism
   assumptions;
3. use `Crumble.jl` when the target is a formal mediation estimand and the
   data support the required assumptions.
...
## 6. Sensitivity to Mediator-Outcome Confounding
...
```{julia}
sensitivity_total_id = identify(sensitivity_graph, :treat, :depress2)
println("total-effect strategy: ", sensitivity_total_id.strategy)
println("Markov pillow of treat: ",
...
@printf("TMLE total effect: %.4f  95%% CI (%.4f, %.4f)\n",
        sensitivity_total_res[:TMLE].ACE,
        sensitivity_total_res[:TMLE].lower_ci,
        sensitivity_total_res[:TMLE].upper_ci)
```
