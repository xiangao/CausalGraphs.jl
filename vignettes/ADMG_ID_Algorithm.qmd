...
The graph below is not p-fixable because the treatment district overlaps with
post-treatment structure, but the nested check still identifies the effect.
This is the kind of case where simple adjustment and front-door logic are not
the right explanation, but fixing operations still provide one.
...
println("a-fixable: ", is_fix(g_hiv, :T))
println("p-fixable: ", is_p_fix(g_hiv, :T))
...
Those functionals are useful identification results. For continuous or
high-dimensional variables, they still require specialized estimation machinery
beyond finite support enumeration.
