...
function backdoor_tmle_a(; a, data::DataFrame, graph::ADMG, treatment, outcome,
    ...
    # One-step / AIPW
    onestep_est = mean((A .== a0) .* (Y .- mu_a) ./ p_a .+ mu_a)
    eif         = (A .== a0) .* (Y .- mu_a) ./ p_a .+ mu_a .- onestep_est
    ...
    gcomp = (estimated_psi=mean(mu_a),                              mu_Y_a=mu_a)
    ipw   = (estimated_psi=mean((A .== a0) .* Y ./ p_a), p_a_mpA=p_a)

    # TMLE targeting
    H = (A .== a0) ./ p_a
    if is_binary(Y)
        ε    = scalar_logistic_fluctuation(Y, safe_logit(mu_a), H)
        mu_t = expit(safe_logit(mu_a) .+ ε .* H)
    else
        ε    = weighted_mean_residual(Y, mu_a, H)
        mu_t = mu_a .+ ε
    end
    tmle_est = mean((A .== a0) .* (Y .- mu_t) ./ p_a .+ mu_t)
    tmle_eif = (A .== a0) .* (Y .- mu_t) ./ p_a .+ mu_t .- tmle_est
